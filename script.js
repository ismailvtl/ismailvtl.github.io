  ...
  request.onsuccess = function(event) {
    ...
    if(request.result) {
      ...
      arr.forEach(data => {
       ...
       exd.innerHTML += `<div class="exd-row"><span>${data.title}</span><span>${data.amount}</span><span>Maturity Date: ${m[2]} ${monthnames[Number(m[1])]} ${m[0]} ( <small>${mon} months left</small> )</span></div>`
      });
    }
  };
}
